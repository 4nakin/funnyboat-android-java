...
public class Enemy extends GameObject {
	...
	static {
		...
		shipImg = BitmapFactory.decodeResource(Game.app.getResources(), R.drawable.merkkari);
		sharkImg = BitmapFactory.decodeResource(Game.app.getResources(), R.drawable.hai);
		mineImg = BitmapFactory.decodeResource(Game.app.getResources(), R.drawable.miina);
	}
	{
		waterUp = true;
		...
		if(isMine) waterUp = false;
		vx = (float) -(Game.water.getWaveSpeed() * 0.5);
		enemy = true;
	}
	Bitmap getImage() {
		if(isMine) return mineImg;
		if(isShark) return sharkImg;
		return shipImg;
	}
	@Override
	public void tick() {
		if(isMine) {
			...
			Game.canvas.drawBitmap(mineImg, x - mineImg.getWidth() / 2, y - 7, null);
			return;
		}
		super.tick();
