...
public class Gameplay implements GameModule {
	...
	public void collide() {
		...
		for(GameObject a: enemies) {
			for(GameObject b: friends) {
				boolean intersect = !(a.x > b.x + b.w || a.x +a.w < b.x || a.y > b.y + b.h || a.y + a.h < b.y);
				if(a instanceof Bullet && b instanceof Bullet) continue;
				if(intersect) {
					if(a.kill())objects.remove(a);
					if(b.kill())objects.remove(b);
				}
			}
		}
	}
	...
	public void tick() {
		...
		for(GameObject ob: new ArrayList<GameObject>(objects)) {
			ob.tick();
			if(ob.y < -10 || ob.x < -10 || ob.x > Game.water.width || ob.y > Game.water.height) {
		...
		if(text != null) {
			...
			Game.canvas.drawText(text, Game.water.width / 2 - size / 2, 40, Menu.textPaint);
	...
	public void touch(MotionEvent ev) {
		if((ev.getAction() & MotionEvent.ACTION_DOWN) != 0 || ev.getAction() == MotionEvent.ACTION_DOWN) {
			if(ev.getY() < Game.water.height / 2) player.shoot();
			else player.jump();
		}
	}
}
