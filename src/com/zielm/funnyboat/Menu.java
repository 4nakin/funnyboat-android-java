...
public abstract class Menu implements GameModule {
	...
	static Bitmap logo;
	static void init() {	
		tf = Typeface.createFromAsset(Game.app.getAssets(),
	            "Cosmetica.ttf");
		logo = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(Game.app.getResources(), R.drawable.logo),
				240, 100, true);
	...
	public void tick() {
		...
		Game.canvas.drawText("zielm.com/android-games/funnyboat (CC-BY-SA and GNU/GPL)", 20, Game.water.height - 30, smallTextPaint);
		Game.canvas.setMatrix(m);
	}
	
	@Override
	public void touch(MotionEvent ev) {
		System.err.println("ev " + ev + " " + ev.getAction() + " " + MotionEvent.ACTION_DOWN + " " + ev.getY());
